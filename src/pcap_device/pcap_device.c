...
static int load_pcap_lib() {
  ...
  if ((m_version < NPCAP_VERSION_M) ||
      ((m_version == NPCAP_VERSION_M) && (n_version < NPCAP_VERSION_N))) {
    wchar_t title[256];

    wsprintf(title, _T("npcap版本号太老: %d.%d < %d.%d"), m_version, n_version,
             NPCAP_VERSION_M, NPCAP_VERSION_N);
    MessageBox(0,
               _T("1.请卸载所有已安装的npcap或者winpcap. \n2. ")
               _T("请安装最新版本npcap，或者安装课程提供的wireshark, ")
               _T("安装过程中安装其附带的npcap."),
...
error_end:
  MessageBox(0,
             ...
             _T("npcap驱动加载失败"), MB_ABORTRETRYIGNORE);
  return -1;
}
...
static int load_pcap_lib() { return 0; }
...
static int pcap_find_device(const char *ip, char *name_buf) {
  ...
  for (item = pcap_if_list; item != NULL; item = item->next) {
    ...
    for (struct pcap_addr *pcap_addr = item->addresses; pcap_addr != NULL;
...
// @param ip 打开网卡的指定ip
// @param 给网卡设置mac
pcap_t *pcapDeviceOpen(const char *ip, const uint8_t *macAddr,
                       uint8_t pollMode) {
  char err_buf[PCAP_ERRBUF_SIZE];
  ...
  sprintf(filter_exp,
          "(ether dst %02x:%02x:%02x:%02x:%02x:%02x or ether broadcast) and "
          "(not ether src %02x:%02x:%02x:%02x:%02x:%02x)",
          macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4],
          macAddr[5], macAddr[0], macAddr[1], macAddr[2], macAddr[3],
          macAddr[4], macAddr[5]);
